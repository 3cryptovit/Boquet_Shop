...
const Constructor = () => {
  ...
  const calculatePriceDetails = () => {
    ...
    // Считаем общую стоимость из деталей
    const totalPrice = Object.values(details).reduce((sum, detail) => sum + detail.totalPrice, 0);
  ...
  const addToCart = async () => {
    ...
    try {
      ...
      if (!token) {
        alert('Для сохранения букета необходимо авторизоваться');
        navigate('/login');
        return;
      }
      ...
      const bouquetData = {
        customBouquet: {
          ...
          price: totalPrice,
          image_url: '/assets/custom-bouquet.jpg',
      ...
      if (!response.ok) {
        throw new Error('Не удалось добавить букет в корзину');
      }

      // Показываем сообщение об успехе
      alert(`Букет "${bouquetData.customBouquet.name}" успешно добавлен в корзину!`);
